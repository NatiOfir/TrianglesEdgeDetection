classdef TrianglesTree < handle
    ...
    methods (Access = public)
        ...
        function obj = updateRespMapPlus(obj,fname) 
...
%             isRegular = ~isSingle;
%             
%             dataSingleLeft = obj.leftTree.respMap(:,absArrNoNan(1,:));
%             pixelsSingleLeft = obj.leftTree.respPixels(:,absArrNoNan(1,:));
%             
%             notSingleLeft = ~isSingleLeft;
%             dataSingleLeft(:,notSingleLeft) = 0;
%             pixelsSingleLeft(:,notSingleLeft) = 0;
%                
%             dataSingleRight = obj.rightTree.respMap(:,absArrNoNan(2,:));
%             pixelsSingleRight = obj.rightTree.respPixels(:,absArrNoNan(2,:));
%             
%             notSingleRight = ~isSingleRight;
%             dataSingleRight(:,notSingleRight) = 0;
%             pixelsSingleRight(:,notSingleRight) = 0;
%             
%             dataSingle = dataSingleRight+dataSingleLeft;
%             pixelsSingle = pixelsSingleRight+pixelsSingleLeft;
%             
%             dataSingle = [dataSingle(obj.Fl,:); dataSingle(obj.Ll,:); dataSingle(obj.Fr,:); dataSingle(obj.Lr,:); -dataSingle(obj.C,:); -dataSingle(obj.maxC,:); -dataSingle(obj.minC,:); dataSingle(obj.NORM,:); dataSingle(obj.S,:)];
...
%             absLeft(:,isSingle) = 1;
%             absRight(:,isSingle) = 1;
%             leftMap = obj.leftTree.respMap(:,absLeft);
%             rightMap = obj.rightTree.respMap(:,absRight);
%             leftMap(:,isSingle) = 0;
%             rightMap(:,isSingle) = 0;
%             fll = obj.leftTree.respMap(obj.Fl,:);
%             fllMap = leftMap(obj.Fl,:);
%             flr = obj.leftTree.respMap(obj.Fr,:);
%             flrMap = leftMap(obj.Fr,:);
%             frl = obj.rightTree.respMap(obj.Fl,:);
%             frlMap = rightMap(obj.Fl,:);
%             frr = rightMap(obj.Fr,:);
%             frrMap = rightMap(obj.Fr,:);
%             fllMap(:,~isRegular) = 0;
%             flrMap(:,~isRegular) = 0;
%             frlMap(:,~isRegular) = 0;
%             frrMap(:,~isRegular) = 0;
%             
%             Lll = obj.leftTree.respMap(obj.Ll,:);
%             LllMap = leftMap(obj.Ll,:);
%             Llr = obj.leftTree.respMap(obj.Lr,:);
%             LlrMap = leftMap(obj.Lr,:);
%             Lrl = obj.rightTree.respMap(obj.Ll,:);
%             LrlMap = rightMap(obj.Ll,:);
%             Lrr = obj.rightTree.respMap(obj.Lr,:);
%             LrrMap = rightMap(obj.Lr,:);
%             LllMap(:,~isRegular) = 0;
%             LlrMap(:,~isRegular) = 0;
%             LrlMap(:,~isRegular) = 0;
%             LrrMap(:,~isRegular) = 0;
...
%             NL = obj.searchSpace(obj.level,obj.N,minLength);
%             T = obj.threshold(obj.sigma,2*w,minLength,NL);
%             score = abs(resp)-T;
%             [maxScore,maxInd] = max(score,[],1);
%             nanScore = isnan(maxScore) | ~isRegular;
%             
...
% 
%             bestMinLeft = obj.leftTree.respMap(obj.minC,bestAbsLeft);
%             bestMaxLeft = obj.leftTree.respMap(obj.maxC,bestAbsLeft);
%             bestMinRight = obj.rightTree.respMap(obj.minC,bestAbsRight);
%             bestMaxRight = obj.rightTree.respMap(obj.maxC,bestAbsRight);
%             bestNormLeft = obj.leftTree.respMap(obj.NORM,bestAbsLeft);
%             bestNormRight = obj.rightTree.respMap(obj.NORM,bestAbsRight);
%             bestPixelsLeft = obj.leftTree.respPixels(:,bestAbsLeft);
%             bestPixelsRight = obj.rightTree.respPixels(:,bestAbsRight);
%             
%             minLeft = bestSignLeft.*bestMinLeft+(~bestSignLeft).*(-bestMaxLeft);
%             maxLeft = bestSignLeft.*bestMaxLeft+(~bestSignLeft).*(-bestMinLeft);
%             minRight = bestSignRight.*bestMinRight+(~bestSignRight).*(-bestMaxRight);
%             maxRight = bestSignRight.*bestMaxRight+(~bestSignRight).*(-bestMinRight);
%             
%             norm = bestNormLeft+bestNormRight;
%             
%             highNorm = norm>4;
%             
%             minContrast = highNorm.*min(minRight,minLeft)+(~highNorm).*maxResp;
%             maxContrast = highNorm.*max(maxRight,maxLeft)+(~highNorm).*maxResp;
%             
%             dataRegular = [maxFilterRight;maxLengthRight;maxFilterLeft;maxLengthLeft;maxResp;minContrast;maxContrast;norm;maxScore];
%             pixelsRegular = [bestPixelsLeft ;bestPixelsRight];
%             dataRegular(:,nanScore) = 0;
%             pixelsRegular(:,nanScore) = 0;
%             obj.respMap = dataRegular+dataSingle;
%             obj.respPixels = pixelsSingle+pixelsRegular;    
        end
   
        function obj = updateRespMap(obj,mex,fname) 
%             prm = getPrm(obj.param);
%             w = prm.filterWidth;
%             load(fname);
%             
%             if mex
%                 [ obj.respMap,obj.respPixels ] = updateRespMapFunc_mex(res,obj.leftTree.respMap,obj.leftTree.respPixels,obj.rightTree.respMap,obj.rightTree.respPixels,obj.respMap,obj.respPixels,obj.sigma,obj.N,obj.level,w,obj.Fl,obj.Ll,obj.Fr,obj.Lr,obj.C,obj.minC,obj.maxC,obj.NORM,obj.S);
%             else
%                 [m,n] = size(res);
%                 for ind = 1:n
%                     curArr = res(:,ind)';
...
%                     if numel(curArr) == 2
%                         if isnan(curArr(2))
%                             data = obj.leftTree.respMap(:,absArr(1));
%                             pixels = obj.leftTree.respPixels(:,absArr(1));
%                         else
%                             data = obj.rightTree.respMap(:,absArr(2));
%                             pixels = obj.rightTree.respPixels(:,absArr(2));
%                         end
%                         
%                         data = [data(obj.Fl) data(obj.Ll) data(obj.Fr) data(obj.Lr) -data(obj.C) -data(obj.maxC) -data(obj.minC) data(obj.NORM) data(obj.S)]';
%                         
%                         obj.respMap(:,ind) = data;
%                         obj.respPixels(:,ind) = [pixels ;zeros(size(pixels))+NaN];
%                     else
%                         absLeft = absArr(1:2:end);
%                         signLeft = signArr(1:2:end);
%                         absRight = absArr(2:2:end);
%                         signRight = signArr(2:2:end);
% 
%                         dataLeft = obj.leftTree.respMap(:,absLeft);
%                         pixelLeft = obj.leftTree.respPixels(:,absLeft);
%                         dataRight = obj.rightTree.respMap(:,absRight);
%                         pixelRight = obj.rightTree.respPixels(:,absRight);
% 
%                         lenGap = 0;
%                         if signLeft(1)>0
%                             filterLeft = dataLeft(obj.Fl,:);
%                             lengthLeft = dataLeft(obj.Ll,:);
%                             filterRight = dataLeft(obj.Fr,:);
%                             lengthRight = dataLeft(obj.Lr,:);
%                         else
%                             filterLeft = dataLeft(obj.Fr,:);
%                             lengthLeft = dataLeft(obj.Lr,:);
%                             filterRight = dataLeft(obj.Fl,:);
%                             lengthRight = dataLeft(obj.Ll,:);
%                         end
%                         
%                         if signRight(1)>0
%                             filterLeft = filterLeft+dataRight(obj.Fl,:);
%                             lengthLeft = lengthLeft+dataRight(obj.Ll,:);
%                             filterRight = filterRight+dataRight(obj.Fr,:);
%                             lengthRight = lengthRight+dataRight(obj.Lr,:);
%                         else
%                             filterLeft = filterLeft+dataRight(obj.Fr,:);
%                             lengthLeft = lengthLeft+dataRight(obj.Lr,:);
%                             filterRight = filterRight+dataRight(obj.Fl,:);
%                             lengthRight = lengthRight+dataRight(obj.Ll,:);
%                         end  
...
%                         
%                         if isempty(bestInd)
%                             obj.respMap(:,ind) = zeros(obj.S,1)+nan;
%                         else
%                             obj.respMap(1:5,ind) = [filterRight(bestInd);lengthRight(bestInd);filterLeft(bestInd);lengthLeft(bestInd);resp(bestInd)];
%                             
%                             if signLeft(1) > 0
%                                 minLeft = dataLeft(obj.minC,bestInd);
%                                 maxLeft = dataLeft(obj.maxC,bestInd);
%                             else
%                                 maxLeft = -dataLeft(obj.minC,bestInd);
%                                 minLeft = -dataLeft(obj.maxC,bestInd);
%                             end
%                             if signRight(1) > 0
%                                 minRight = dataRight(obj.minC,bestInd);
%                                 maxRight = dataRight(obj.maxC,bestInd);
%                             else
%                                 maxRight = -dataRight(obj.minC,bestInd);
%                                 minRight = -dataRight(obj.maxC,bestInd);
%                             end
% 
%                             obj.respMap(obj.NORM,ind) = dataLeft(obj.NORM,bestInd)+dataRight(obj.NORM,bestInd);
%                             
%                             if obj.respMap(obj.NORM,ind) > 4
%                                 obj.respMap(obj.minC,ind) = min(minLeft,minRight);
%                                 obj.respMap(obj.maxC,ind) = max(maxLeft,maxRight);
%                             else 
%                                 obj.respMap(obj.minC,ind) = maxResp;
%                                 obj.respMap(obj.maxC,ind) = maxResp;
%                             end
%                             
%                             
%                             obj.respPixels(:,ind) = [pixelLeft(:,bestInd) ;pixelRight(:,bestInd)];
%                         end
%                     end
%                 end       
%             end
        ...
        function obj = updateRespMapFirstLevel(obj,I)
            ...
            %coords = bsxfun(@plus,coords,[obj.p1(1);obj.p1(2);obj.p1(1);obj.p1(2)]);
            %coordsInd = sub2ind(size(Ior),coordsx,coordsy);
            %coordsInd(2:end-1,:) = [];
            %obj.respMap(obj.S+1:obj.S+2,:) = coordsInd;
            obj.respPixels = sub2ind(size(Ior),coordsx,coordsy);
            %obj.respPixels = obj.respPixels([1,end],:);
            %obj.respPixels = mat2cell(obj.respPixels,2,ones(1,n));
            ...
            %I(isnan(Y)) = [];
            %I = I(end:-1:1);
            %m1 = find(scores(1:gap) == max(scores(1:gap)));
            %m2 = find(scores(gap+1:2*gap) == max(scores(gap+1:2*gap)))+gap;
            %m3 = find(scores(2*gap+1:3*gap) == max(scores(2*gap+1:3*gap)))+2*gap;
            %m = false(size(scores));
            %m(I(1:30)) = true;
            %m([m1 m2 m3]) = true;
            %m1
    ...
    methods (Static)
        function  [respMapNew respPixelsNew]= updateRespMapPlusPlus(respMap,param,length,lengthHypo,blockSize,level,N,sigma) 
            ...
            %newBlockSize = size(signArr,2);
            
            %signArrNew = zeros(size(signArr,1),newBlockSize*blocks);
            %absArrNew = zeros(size(absArr,1),newBlockSize*blocks);
            ...
            %for i=1:blocks
            %    signArrNew(:,(i-1)*newBlockSize+1:i*newBlockSize) = signArr;
            %    absArrNew(:,(i-1)*newBlockSize+1:i*newBlockSize) = absArr+(i-1)*2*blockSize;
            %end
            
            resNew = absArrNew.*signArrNew; 
            
            %respMap = gpuArray(respMap);
            %respPixels = gpuArray(respPixels);
            %resNew = gpuArray(resNew);
            ...
            %bestPixelsLeft = respMap(TrianglesTree.S+1:end,bestAbsLeft);
            %bestPixelsRight = respMap(TrianglesTree.S+1:end,bestAbsRight);
            %bestPixelsLeft = respPixels(:,bestAbsLeft);
            %bestPixelsRight = respPixels(:,bestAbsRight);
