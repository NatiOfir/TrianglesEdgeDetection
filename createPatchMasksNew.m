function createPatchMasks(param)
    prm = getPrm(param);
    S = prm.minLength;
    w = prm.filterWidth;
    ...
    for edge1 = 0:1
        for edge2 = edge1+1:2
            [bound,inTriangle] = getInTriangle(edge1,edge2,S,w);
            bound = bound*0.5;
            bound(bound ==0) = 1;
            for v1 = 0:S-1
                for v2 = 0:S-1
                    ...
                    if edge1 == 0 && edge2 == 1
                        for k = 1:w
                            lineLeft = Line(x0+k+gap,y0+gap,x1+gap,y1-k+gap,1);
                            lineRight = Line(x0-k+gap,y0+gap,x1+gap,y1+k+gap,1);
                            %lineRight = Line(x1+gap,y1+k+gap,x0-k+gap,y0+gap,1);
                            curMask = curMask+lineLeft.getLineImage(S+2*w,S+2*w)-lineRight.getLineImage(S+2*w,S+2*w);
                        end
                    elseif edge1 == 0 && edge2 == 2
                        for k = 1:w
                            lineLeft = Line(x0+k+gap,y0+gap,x1+k+gap,y1+k+gap,1);
                            lineRight = Line(x0-k+gap,y0+gap,x1-k+gap,y1-k+gap,1);
                            curMask = curMask+lineLeft.getLineImage(S+2*w,S+2*w)-lineRight.getLineImage(S+2*w,S+2*w);
                        end
                    elseif edge1 == 1 && edge2 == 2
                        for k = 1:w
                            lineLeft = Line(x0+gap,y0-k+gap,x1-k+gap,y1-k+gap,1);
                            lineRight = Line(x0+gap,y0+k+gap,x1+k+gap,y1+k+gap,1);
                            curMask = curMask+lineLeft.getLineImage(S+2*w,S+2*w)-lineRight.getLineImage(S+2*w,S+2*w);
                        end
                    end
                    line1 = Line(x0+gap,y0+gap,x1+gap,y1+gap,1);
                    curEdge = line1.getLineImage(S+2*w,S+2*w);
...
function [bound,inTriangle] = getInTriangle(edge1,edge2,S,w)
    L = S+2*w;
    inTriangle = zeros(L);
    bound = zeros(L);
